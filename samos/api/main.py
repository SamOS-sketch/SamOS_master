...
def export_events(
    ...
    return list_events(
        session_id=session_id, kind=kind, since=since, until=until, limit=limit
    )
    ...
    try:
        ...
        try:
            ...
            result = provider.generate(
                session_id=req.session_id, prompt=req.prompt, reference_image=reference
            )
            ...
            # Event + metrics
            record_event(
                "image.generate.fail", "Image failed", req.session_id, {"error": str(e)}
            )
