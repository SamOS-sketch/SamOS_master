...
# Ensure root .env is loaded (two levels up from this file)
...
from samos.core.memory_agent import get_memory_agent  # MemoryAgent
...
_provider_factories = {"stub": lambda: StubProvider(), "openai": _make_openai_provider}
_provider_cache = {}
def get_provider():
...
def health():
    ...
    return {"status": "ok", "provider": provider, "soulprint_path": SOULPRINT_PATH, "soulprint_name": sp_name}
...
def list_events(
    ...
    try:
        q = db.query(DBEvent)
        if session_id: q = q.filter(DBEvent.session_id == session_id)
        if kind: q = q.filter(DBEvent.kind == kind)
        sdt = _parse_iso(since);  edt = _parse_iso(until)
        if sdt: q = q.filter(DBEvent.ts >= sdt)
        if edt: q = q.filter(DBEvent.ts <= edt)
        rows = q.order_by(DBEvent.id.desc()).limit(limit).all()
        return [{"id": r.id, "ts": r.ts.isoformat() if r.ts else None, "session_id": r.session_id,
                 "kind": r.kind, "message": r.message, "meta": json.loads(r.meta_json or "{}")} for r in rows]
    finally:
        db.close()

@app.get("/events/export")
def export_events(session_id: Optional[str] = Query(None), kind: Optional[str] = Query(None),
...
def put_memory(req: MemoryPutRequest):
    ...
    try:
        sess = db.query(DBSession).filter(DBSession.id == req.session_id).first()
        if not sess: raise HTTPException(status_code=404, detail="Session not found")
        item = (db.query(DBMemory)
                  .filter(DBMemory.session_id == req.session_id, DBMemory.key == req.key)
                  .first())
        if not item:
            item = DBMemory(session_id=req.session_id, key=req.key, value=req.value,
...
def list_memory(session_id: str = Query(...)):
    ...
    try:
        ...
        return MemoryListResponse(items=[
            MemoryItem(key=i.key, value=i.value, meta=json.loads(i.meta_json or "{}"),
                       created_at=i.created_at.isoformat() + "Z",
                       updated_at=i.updated_at.isoformat() + "Z") for i in items
        ])
    finally:
        db.close()
...
def create_emm(req: EMMCreateRequest):
    ...
    try:
        sess = db.query(DBSession).filter(DBSession.id == req.session_id).first()
        if not sess: raise HTTPException(status_code=404, detail="Session not found")
        e = DBEMM(session_id=req.session_id, type=req.type, message=req.message,
                  meta_json=json.dumps(req.meta or {}))
        db.add(e); db.commit()
...
def list_emms(session_id: str = Query(...), limit: int = Query(50)):
    ...
    try:
        rows = (db.query(DBEMM).filter(DBEMM.session_id == session_id)
                .order_by(DBEMM.id.desc()).limit(limit).all())
        return EMMListResponse(items=[
            EMMItem(id=r.id, type=r.type, message=r.message,
...
def generate_image(req: ImageGenerateRequest):
    ...
    try:
        ...
        try:
            result = provider.generate(session_id=req.session_id, prompt=req.prompt, reference_image=reference)
            img = DBImage(
                id=result["image_id"], session_id=req.session_id, prompt=req.prompt,
                provider=result["provider"], url=result["url"],
                reference_used=result["reference_used"], status=result.get("status", "ok"),
                meta_json=json.dumps(result.get("meta", {})),
            )
            db.add(img); db.commit()
            record_event("image.generate.ok", "Image created", req.session_id,
                         {"provider": result.get("provider"), "image_id": result.get("image_id")})
            _METRICS["image.ok"] += 1
            ts = _utc_now(); _bump_buckets("image.ok", ts)
            try:
                AGENT.on_event(req.session_id, "image.generate.ok", "Image created",
            ...
            emm = DBEMM(session_id=req.session_id, type="OneBounce", message=str(e),
